...
class SlackBotTool(BaseTool):
    """Post messages through a Slack incoming webhook using plain HTTP."""

    name: str = "Slack News Publisher"
    description: str = "Posts each news summary to a configured Slack channel."
    ...
    def _run(self, summaries_json: str | list[Any] | dict[str, Any]) -> str:
        ...
        for summary in summaries:
            message = (
                ...
                f"<{summary.source_url}|Read the full article>"
            )
            if dry_run:
                print(f"[SLACK_DRY_RUN] {message}")
